...
class Search extends Component {
    ...
    handleClick=(event)=>{
      ...
      this.setState({result_calendar: calendar.filter(function(c){
            let val = 0;
    ...
    this.setState({result_contacts: contacts.filter(function(c){
        let val=0;
        c.matching_terms.forEach(item=>
            {
                if(query.toLowerCase().indexOf(item.toLowerCase()) !== -1){
    ...
    this.setState({result_dropbox: dropbox.filter(function(c){
        let val=0;
            c.matching_terms.forEach(item=>
                {
                    if(query.toLowerCase().indexOf(item.toLowerCase()) !== -1){
    ...
    this.setState({result_slack: slack.filter(function(c){
        let val=0;
            c.matching_terms.forEach(item=>
                {
                    if(query.toLowerCase().indexOf(item.toLowerCase()) !== -1){
    ...
    this.setState({result_tweet: tweet.filter(function(c){
        let val=0;
            c.matching_terms.forEach(item=>
                {
                    if(query.toLowerCase().indexOf(item.toLowerCase()) !== -1){
